...
public class PlatformCatcher : MonoBehaviour
{
    ...
    public class CaughtObject
    {
        ...
        public void Move(Vector2 movement)
        {
            if (!inContact)
            {
                character.movingViaPlatform = false;
                return;
            }

            if (character != null)
                character.movingViaPlatform = true;
            //     character.MoveOnPlatform(movement);
            // else
    ...
    void Awake()
    {
        ...
        //if we have a parent platform catcher, we make it's move "bubble down" to that catcher, so any object caught by that platform catcher will also
        ...
        do
        {
            for (int i = 0, count = m_CaughtObjects.Count; i < count; i++)
            {
                ...
                if(!caughtObject.inContact)
                {
                    ...
                    bool verticalAlignment = (caughtObjectCollider.bounds.max.x > m_Collider.bounds.min.x) && (caughtObjectCollider.bounds.min.x < m_Collider.bounds.max.x);
    ...
    public bool HasCaughtObject(GameObject obj)
    {
        for (int i = 0; i < m_CaughtObjects.Count; i++)
        {
            if (m_CaughtObjects[i].collider.gameObject == obj && m_CaughtObjects[i].inContact)
                return true;
        }
